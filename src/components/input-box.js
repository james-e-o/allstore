const InputBox = ({label,id,placeholder,flexdir,type,width,margin,change,inputDir,blurr,value,note,textarea,grow,row,icon,readonly,error,outline,shortInput,ghost,mt,fit,uppercase,input_uppercase,font,l_font,input,mr})=>{
  ...
  return(
    <div data-dir={!!flexdir} data-fit={fit} data-error={error} data-mt={mt} data-mr={mr} data-grow={grow} data-short={shortInput} style={{width:`${width}`,flexDirection:flexdir,margin:margin}} className={`relative w-full data-[error=true]:mb-1 data-[mt=true]:mt-2 data-[mr=true]:mr-3 text-ellipsis data-[grow=true]:flex-grow mt-0 data-[dir=true]:items-center data-[fit=true]:w-fit gap-1 inline-flex flex-col`}>
      {label?<label data-dir={!!flexdir} data-font={l_font} data-uppercase={uppercase} className="pl-1 data-[dir=true]:pl-0 data-[uppercase=true]:uppercase relative inline-flex items-center w-fit justify-start data-[font=base]:text-base data-[font=sm]:text-sm text-ellipsis md:min-w-max" htmlFor="">{label}</label>:""}
