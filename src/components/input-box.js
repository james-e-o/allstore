...
const InputBox = ({label,placeholder,error,flexdir,type,width,margin,change,inputDir,blurr,value,note,textarea,grow,row,icon,readonly,outline,shortInput,ghost,mt,fit,uppercase,input_uppercase,font,l_font,input,mr})=>{
  return(
    <div data-dir={!!flexdir} data-fit={fit} data-mt={mt} data-mr={mr} data-grow={grow} data-short={shortInput} style={{width:`${width}`,flexDirection:flexdir,margin:margin}} className={`relative w-full data-[mt=true]:mt-2 data-[mr=true]:mr-3 text-ellipsis data-[grow=true]:flex-grow mt-0 data-[dir=true]:items-center data-[fit=true]:w-fit gap-1 inline-flex flex-col`}>
