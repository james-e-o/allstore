  ...
  return (
    <div className="h-full pt-1 pl-2 w-full">
      ...
      <div className="grid grid-cols-[repeat(5,minmax(1rem,1fr))_max-content] grid-rows-[max-content_min-content_repeat(2,minmax(1rem,1fr))_max-content] gap-3">
        ...
        <div className="justify-items-stretch mt-3 col-start-1 col-span-5 row-start-3 row-span-3 gap-2  items-start lg:grid-cols-2 md:grid-cols-1 inline-grid">
...
export const LongCardbox = () => {
  return (
    <div className="py-4 px-2 bg-white shadow rounded-md flex flex-grow h-full flex-col gap-2 justify-between">
      <p className="text-9px text-core_contrast/60 text-ellipsis">
        <span className="mr-5 relative -top-2">Sales overview</span>
      </p>
      ...
      <p className="text-9px font-semibold flex flex-col">
        <span className="text-green-500 items-center text-10px inline-flex"><ArrowUpRight className="w-3 h-3"/>{`3.2`}%</span>
        <span>From last week</span>
      </p>
    </div>
  )
